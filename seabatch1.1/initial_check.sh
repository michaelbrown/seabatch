...
###########################################################################
###########################################################################
#Read in variables from master_batch_processing_script.sh:
...
TMPRL_AVG_AMNT=${10}
for (( b=0; b < $TMPRL_AVG_AMNT; b++ )); do
	PARAMETER=$(( 16 + $L2GEN_PRDCT_AMNT + $b + 1))
	eval TMPRL_AVG_ARRAY[$b]=\${$PARAMETER}
done

L2BIN_RES=${11}

OUT_PRDCT_AMNT=${12}
for (( c=0; c < $OUT_PRDCT_AMNT; c++ )); do
	PARAMETER=$(( 16 + $L2GEN_PRDCT_AMNT + $TMPRL_AVG_AMNT +$c + 1))
	eval OUT_PRDCT_ARRAY[$c]=\${$PARAMETER}
done

OUT_FTYPE_AMNT=${13}
for (( d=0; d < $OUT_FTYPE_AMNT; d++ )); do
	PARAMETER=$(( 16 + $L2GEN_PRDCT_AMNT + $TMPRL_AVG_AMNT + $OUT_PRDCT_AMNT + $d + 1))
	eval OUT_FTYPE_ARRAY[$d]=\${$PARAMETER}
done

CT_SETTING=${14}

CT_SUBDIR_AMNT=${15}
for (( e=0; e < $CT_SUBDIR_AMNT; e++ )); do
	PARAMETER=$(( 16 + $L2GEN_PRDCT_AMNT + $TMPRL_AVG_AMNT + $OUT_PRDCT_AMNT + $OUT_FTYPE_AMNT + $e + 1))
	eval CT_SUBDIR_ARRAY[$e]=\${$PARAMETER}
done

CT_NUM_AMNT=${16}
for (( f=0; f < $CT_NUM_AMNT; f++ )); do
	PARAMETER=$(( 16 + $L2GEN_PRDCT_AMNT + $TMPRL_AVG_AMNT + $OUT_PRDCT_AMNT + $OUT_FTYPE_AMNT + $CT_SUBDIR_AMNT + $f + 1))
	eval CT_NUM_ARRAY[$f]=\${$PARAMETER}
done
###########################################################################
###########################################################################
...
if [ $END_LEVEL -eq 4 ]; then
	...
	if [ $OUT_FTYPE_AMNT -gt 0 ]; then
		...
		#file type is valid (asc, flat, hdf, map, or png):
		for (( i=0; i < $OUT_FTYPE_AMNT; i++ )); do
			case ${OUT_FTYPE_ARRAY[$i]} in 
				...
				;;
				*) 
				echo; echo; echo 'ERROR:' ${OUT_FTYPE_ARRAY[$i]} 'is not a valid output file type (Must be flat or png)'
				exit 1
				;;
			esac
		done
